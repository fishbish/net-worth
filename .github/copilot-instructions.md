...
- The core data model is:
  ...
  - `Instrument` -> child record under an `Account`
  - `AccountSnapshot` -> dated account snapshot header that stores either an account balance or grouped instrument balances
  - `InstrumentSnapshot` -> dated balance for one instrument within an account snapshot
...
- `NetWorthDbContext` sets all foreign keys to `DeleteBehavior.NoAction` first, then explicitly enables cascade delete only for `Instrument -> Account`.
...
- Treat small catalog entities such as `Institution` as direct CRUD over the EF entity, including direct UI binding and bulk `SaveAllAsync` flows.
